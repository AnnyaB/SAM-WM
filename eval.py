...
DATASET_SOURCES = {
    ...
    "novisad": "doi:10.5281/zenodo.7738094",
...
def claim_heldout(receipt: Path, payload: dict) -> None:
    """Atomically authorize held-out metric computation once per output directory."""
    ...
    try:
        ...
        raise SystemExit(
            f"held-out receipt already exists: {receipt}; refusing repeated evaluation"
        ) from exc
...
def _heldout_receipt(
    ...
    *,
    ...
    claim_heldout(
        out / f"{dataset}_HELDOUT_OPEN.json",
        {
            "status": "OPENED_BEFORE_HELDOUT_METRIC_COMPUTATION",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate a frozen SAM-WM checkpoint with an auditable held-out gate."
    )
    parser.add_argument("--config", default="config/train.yaml")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument(
        "--data",
        choices=["freiburg", "novisad", "fairurbtemp"],
        required=True,
    )
    ...
    if args.data == "freiburg":
        ds = load_freiburg(args.root or cfg["data_root"], k=graph_k)
        ...
        ds = load_novisad(args.root or "data/novisad", k=graph_k)
        source = load_freiburg(cfg["data_root"], k=graph_k)
        ...
        ds = load_fairurbtemp(args.root, city=args.city, k=graph_k)
        source = load_freiburg(cfg["data_root"], k=graph_k)
        radius = calibration_from_split(source, cfg, args.checkpoint)
        split = (str(ds.timestamps[48]), str(ds.timestamps[-1]))
    ...
    evaluation = f"{args.data}_{args.split}"
    save_manifest(ds, out / f"{evaluation}_manifest.json")
    payload = {
    ...
    metrics_path = out / f"{evaluation}_metrics.json"
