...
from .experiment import evaluate_split, make_starts, train_model
from .samwm import EDGE_DIM, SAMWMOutput, SAMWorldModel, SIGReg
...
def _temperature_only_loss(
    ...
    """Control objective: temperature likelihood + SIGReg, without latent prediction."""
...
def research_runtime(loss_variant: str = "standard") -> Iterator[None]:
    """Install research-only variants temporarily and restore production functions afterward."""
...
def evaluate_sanity_baselines(
    ...
    """Validation-only sanity baselines under the exact SAM-WM window contract."""
...
def run_stage(ds: UrbanDataset, cfg: dict[str, Any], root: Path, stage: str) -> None:
    if stage == "baselines":
        payload = {
            "split": "validation",
            "heldout_or_ood_accessed": False,
