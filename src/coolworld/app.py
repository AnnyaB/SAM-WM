...
def readiness() -> dict[str, Any]:
    ...
    context_ready = len(frames) >= 48
    counterfactual_ready = checkpoint_ready and calibration_ready and candra_ready and context_ready
    if counterfactual_ready:
        status = "READY_EVIDENCE_BOUNDED"
    ...
    return {
        ...
        "world_modes": {
            ...
            "counterfactual": "requires checkpoint + calibration + compatible real context + CANDRA evidence",
        },
    }
...
def fortyguard_heatmap(payload: dict[str, Any] = Body(...)) -> dict[str, Any]:
    if not os.getenv("FORTYGUARD_API_KEY"):
        raise HTTPException(status_code=503, detail="DATA_UNAVAILABLE: FORTYGUARD_API_KEY not configured")
    try:
...
def counterfactual(_: dict[str, Any] = Body(...)) -> dict[str, Any]:
    state = readiness()["counterfactual_model"]
    if not state["ready"]:
        raise HTTPException(status_code=409, detail=state["status"])
    raise HTTPException(
        status_code=409,
        detail="COUNTERFACTUAL_ENGINE_NOT_PROMOTED: validate Kaggle checkpoint and CANDRA artifact first",
    )
